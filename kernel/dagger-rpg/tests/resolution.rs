...
// Test-only immutable copy of the same runtime package admitted by Product
// Assembly. The test never reaches the product root or a filesystem path.
const PACKAGE: &[u8] = include_bytes!("fixtures/dagger-core.package.json");
...
fn embedded_composed_expressions_retain_admitted_parent_and_strict_paths() {
    ...
    let package: rusty_engine::product_kernel::serde_json::Value =
        rusty_engine::product_kernel::serde_json::from_slice(PACKAGE).expect("parse package");
...
fn injected_rule_rejects_a_tagged_action_while_condition_without_mutating() {
    let package: rusty_engine::product_kernel::serde_json::Value =
        rusty_engine::product_kernel::serde_json::from_slice(PACKAGE)
            .expect("parse committed package");
    let mut mutated = package.clone();
    mutated["payload"]["rules"].as_array_mut().unwrap().push(
        rusty_engine::product_kernel::serde_json::json!({
            "id": "fatigue-lockout",
    ...
    let catalog = compile_gameplay_package(
        &rusty_engine::product_kernel::serde_json::to_vec(&mutated)
            .expect("encode mutated package"),
    )
...
fn binary64_behavior_tuning_crosses_admission_at_one_f32_boundary() {
    let package: rusty_engine::product_kernel::serde_json::Value =
        rusty_engine::product_kernel::serde_json::from_slice(PACKAGE)
            .expect("parse committed package");
    let compile_with = |patch: &[&str], value: rusty_engine::product_kernel::serde_json::Value| {
        let mut mutated = package.clone();
    ...
    let catalog = compile_with(
        ...
        rusty_engine::product_kernel::serde_json::Value::from(0.1),
    )
    ...
    let catalog = compile_with(
        &["payload", "actors", "2", "behavior", "patrolSpeed"],
        rusty_engine::product_kernel::serde_json::Value::from(1.5),
    )
    ...
    let catalog = compile_with(
        ...
        rusty_engine::product_kernel::serde_json::Value::from(0.005),
    )
    ...
    let catalog = compile_with(
        &["payload", "actors", "2", "behavior", "patrolSpeed"],
        rusty_engine::product_kernel::serde_json::Value::from(-0.0),
    )
    ...
    assert!(matches!(
        compile_with(
            &["payload", "actors", "2", "behavior", "detectionRange"],
            rusty_engine::product_kernel::serde_json::Value::from(2000.0)
        ),
    ...
    assert!(matches!(
        compile_with(
            &["payload", "actors", "2", "behavior", "detectionRange"],
            rusty_engine::product_kernel::serde_json::Value::from(0.0005)
        ),
        Err(DaggerGameplayError::InvalidValue { .. })
    ));
    assert!(matches!(
        compile_with(
            &["payload", "actors", "2", "behavior", "detectionRange"],
            rusty_engine::product_kernel::serde_json::Value::from(f64::NAN)
        ),
    ...
    let catalog = compile_with(
        ...
        rusty_engine::product_kernel::serde_json::Value::from(4.75),
    )
...
fn admission_rejects_undeclared_vocabulary_and_dangling_references() {
    let package: rusty_engine::product_kernel::serde_json::Value =
        rusty_engine::product_kernel::serde_json::from_slice(PACKAGE)
            .expect("parse committed package");
    let encode = |mutated: rusty_engine::product_kernel::serde_json::Value| {
        rusty_engine::product_kernel::serde_json::to_vec(&mutated).expect("encode mutated package")
    };

    // Undeclared stat id in an actor definition.
    let mut mutated = package.clone();
    mutated["payload"]["actors"][0]["stats"]["stealthiness"] =
        rusty_engine::product_kernel::serde_json::Value::from(50);
    assert!(matches!(
    ...
    let mut mutated = package.clone();
    mutated["payload"]["actors"][2]["behavior"]["action"] =
        rusty_engine::product_kernel::serde_json::Value::from("rat-explode");
    assert!(matches!(
        compile_gameplay_package(&encode(mutated)),
        Err(DaggerGameplayError::InvalidValue { .. })
    ));
    ...
    let mut mutated = package.clone();
    mutated["payload"]["actors"][2]["tracks"][0]["max"]["tree"] = rusty_engine::product_kernel::serde_json::json!({
        "op": "input", "input": {
...
fn track_reads_reject_where_live_values_do_not_exist() {
    ...
    let package: rusty_engine::product_kernel::serde_json::Value =
        rusty_engine::product_kernel::serde_json::from_slice(PACKAGE)
            .expect("parse committed package");
    let mut mutated = package.clone();
    mutated["payload"]["actors"][0]["tracks"][0]["max"]["tree"] = rusty_engine::product_kernel::serde_json::json!({
...
fn policy_accepts_declared_arbitrary_unbounded_evidence_but_rejects_unknown_bounded_extra() {
    let package = mutated_package(|package| {
        package["payload"]["actions"][0]["program"]["steps"][1]["predicate"]["left"]["tree"]
            ["input"]["id"] = rusty_engine::product_kernel::serde_json::Value::String(
            "melee-attack.future-roll".to_string(),
        );
    });
...
fn empty_action_plan_accepts_declared_unbounded_samples_but_rejects_unknown_samples() {
    fn remove_bounded_and_product_inputs(
        value: &mut rusty_engine::product_kernel::serde_json::Value,
    ) {
        let is_product = value
            ...
            == Some("product");
        if is_product {
            *value = rusty_engine::product_kernel::serde_json::json!({"op": "literal", "value": 0});
            return;
        }
        if let Some(object) = value.as_object_mut() {
            let is_bounded_roll = object
                .get("kind")
                .and_then(rusty_engine::product_kernel::serde_json::Value::as_str)
                == Some("boundedRoll");
            if is_bounded_roll {
                object.insert(
                    "kind".to_string(),
                    rusty_engine::product_kernel::serde_json::Value::from("roll"),
                );
...
fn mutated_package(
    patch: impl FnOnce(&mut rusty_engine::product_kernel::serde_json::Value),
) -> Vec<u8> {
    let mut package: rusty_engine::product_kernel::serde_json::Value =
        rusty_engine::product_kernel::serde_json::from_slice(PACKAGE)
            .expect("parse committed package");
    patch(&mut package);
    rusty_engine::product_kernel::serde_json::to_vec(&package).expect("encode mutated package")
}
...
fn equippable_items_without_an_equipment_section_reject() {
    ...
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["equipment"]["slots"] =
                rusty_engine::product_kernel::serde_json::json!([]);
        })),
        Err(DaggerGameplayError::InvalidValue { .. })
    ));
    ...
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["equipment"]["capacityMetrics"] =
                rusty_engine::product_kernel::serde_json::json!([]);
        })),
        Err(DaggerGameplayError::InvalidValue { .. })
    ));
    ...
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["actors"][0]["inventory"][0]["item"] =
                rusty_engine::product_kernel::serde_json::Value::from("mithril-ladle");
        })),
        Err(DaggerGameplayError::InvalidValue { .. })
    ));
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["actors"][0]["inventory"][0]["equipSlot"] =
                rusty_engine::product_kernel::serde_json::Value::from("third-hand");
        })),
        Err(DaggerGameplayError::InvalidValue { .. })
    ));
    ...
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["actors"][0]["inventory"][gold_index]["equipSlot"] =
                rusty_engine::product_kernel::serde_json::Value::from("left-hand");
        })),
        Err(DaggerGameplayError::InvalidValue { .. })
    ));
}
...
fn invalid_hands_material_and_interceptor_fields_reject() {
    ...
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["items"][0]["weapon"]["hands"] =
                rusty_engine::product_kernel::serde_json::Value::from("three");
        })),
    ...
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["items"][0]["weapon"]["material"] =
                rusty_engine::product_kernel::serde_json::Value::from("glass");
        })),
        Err(DaggerGameplayError::InvalidValue { .. })
    ));
    // Unknown armor material (no table entry).
    let cuirass = |package: &rusty_engine::product_kernel::serde_json::Value| {
    ...
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["items"][index]["interceptor"] = rusty_engine::product_kernel::serde_json::json!({ "kind": "reduceDamage", "amount": 1 });
        })),
        Err(DaggerGameplayError::Payload(_))
    ));
}
...
fn equip_legality_state() -> (dagger_rpg::DaggerGameplayCatalog, DaggerGameplayState) {
    let package = mutated_package(|package| {
        package["payload"]["actors"][0]["inventory"] = rusty_engine::product_kernel::serde_json::json!([
            { "item": "iron-staff" },
            { "item": "buckler" },
            { "item": "iron-dagger" },
            { "item": "iron-wakazashi" },
...
fn loot_table_admission_rejects_bad_values() {
    ...
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["lootTables"][1]["categories"]["armor"] =
                rusty_engine::product_kernel::serde_json::json!(101);
        })),
        Err(DaggerGameplayError::InvalidValue { .. })
    ));
    ...
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["lootTables"][1]["gold"]["min"] =
                rusty_engine::product_kernel::serde_json::json!(11);
        })),
        Err(DaggerGameplayError::InvalidValue { .. })
    ));
    // Duplicate key.
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["lootTables"][1]["key"] =
                rusty_engine::product_kernel::serde_json::Value::from("-");
        })),
        Err(DaggerGameplayError::DuplicateId { .. })
    ));
    ...
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["lootTables"][0]["surprise"] =
                rusty_engine::product_kernel::serde_json::json!(1);
        })),
        Err(DaggerGameplayError::Payload(_))
    ));
    ...
    for bad_key in ["a", "AB", "1"] {
        assert!(matches!(
            compile_gameplay_package(&mutated_package(|package| {
                package["payload"]["lootTables"][1]["key"] =
                    rusty_engine::product_kernel::serde_json::Value::from(bad_key);
            })),
            Err(DaggerGameplayError::InvalidValue { .. })
        ));
    }
...
fn loot_category_terminates_at_the_first_failed_roll() {
    let catalog = compile_gameplay_package(PACKAGE).expect("compile authored Dagger package");
    // Fail-first case: table A weapons chance 5 — slot 0 fails (50 !< 5),
    // so slot 1 must not be evaluated even though its roll (0) would
    // succeed at the halved chance 2.
    let generation = generate_loot(
        &catalog,
        "A",
        1,
        &loot_evidence(&catalog, "A", &[("loot.A.weapons.0", 50)]),
    )
    .expect("generation");
    let weapons = generation
        .categories
    ...
    // evaluated even though its roll (0) would succeed at chance 25.
    let generation = generate_loot(
        &catalog,
        "H",
        1,
...
fn progression_admission_validates_rewards_and_rejects_unknown_fields() {
    ...
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["actors"][2]["xpReward"] =
                rusty_engine::product_kernel::serde_json::json!(-1);
        })),
        Err(DaggerGameplayError::InvalidValue { .. })
    ));
    ...
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["actors"][0]["hitPointsPerLevel"] =
                rusty_engine::product_kernel::serde_json::json!(-1);
        })),
        Err(DaggerGameplayError::InvalidValue { .. })
    ));
    ...
    assert!(matches!(
        compile_gameplay_package(&mutated_package(|package| {
            package["payload"]["actors"][2]["xpBounty"] =
                rusty_engine::product_kernel::serde_json::json!(1);
        })),
        Err(DaggerGameplayError::Payload(_))
    ));
