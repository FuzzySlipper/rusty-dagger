...
impl<'de> Deserialize<'de> for Binary64I64 {
    ...
    where
        D: rusty_engine::product_kernel::serde::Deserializer<'de>,
    {
        let number = rusty_engine::product_kernel::serde_json::Number::deserialize(deserializer)?;
        if let Some(value) = number.as_i64() {
            return Ok(Self(value));
        }
        let float = number.as_f64().ok_or_else(|| {
            rusty_engine::product_kernel::serde::de::Error::custom("expected a number")
        })?;
        if float.fract() == 0.0 && float >= i64::MIN as f64 && float <= i64::MAX as f64 {
            Ok(Self(float as i64))
        } else {
            Err(rusty_engine::product_kernel::serde::de::Error::custom(
                format!("expected an integral number, got {float}"),
            ))
        }
    }
}
...
pub struct AuthoredDerivedRule {
    ...
    pub expr: rusty_engine::product_kernel::serde_json::Value,
}
...
pub struct AuthoredTrackDefinition {
    pub id: String,
    pub max: rusty_engine::product_kernel::serde_json::Value,
}
...
pub enum AuthoredPredicate {
    Cmp {
        op: AuthoredCmpOp,
        left: rusty_engine::product_kernel::serde_json::Value,
        right: rusty_engine::product_kernel::serde_json::Value,
    },
}
...
pub enum AuthoredOperation {
    SpendTrack {
        track: String,
        amount: rusty_engine::product_kernel::serde_json::Value,
    },
    Damage {
