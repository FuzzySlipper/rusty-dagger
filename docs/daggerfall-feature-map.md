...
- **Kit `ActorsState` vs Engine `Entities/EntityStore`.** `EntityStore` is product-owned component storage (revisions, batches, snapshots); `ActorsState` is a deliberately small lifecycle map over Mechanics identities. Non-use of `EntityStore` is allowed by the capabilities map ("a `using` declaration is enough to ignore a helper that is irrelevant"), not a fork. Revisit only if actor storage needs revisions/batches/snapshots.
- **Kit `World/DurableIdentity.cs` vs Engine `Entities/EntityStore`.** Closest Engine shape: `EntityStore` also carries a monotonic non-reusing cursor, a tombstone lifecycle, and a validated restore path over product-supplied identity evidence. It does not apply here, because the identities Daggerfall governs are caller-chosen content values handed to Mechanics as opaque ids; `EntityStore` admits chosen ids only by replacing its whole managed world, and it models no authored reservations, no per-kind namespaces, and no save schema (which it leaves to the product). Adopting it would add component, containment, and revision machinery to an id space the Engine never issued. Revisit only if the product begins using `EntityStore` for its own entity storage.
