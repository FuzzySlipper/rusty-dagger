...
- **Kit `World/DurableIdentity.cs` vs Engine `Entities/EntityWorld`.** Closest Engine shape: `EntityWorld` also carries a monotonic non-reusing cursor, a tombstone lifecycle, and a validated restore path over product-supplied identity evidence. It does not apply here, because the identities Daggerfall governs are caller-chosen content values handed to Mechanics as opaque ids; `EntityWorld` admits chosen ids only by replacing its whole managed world, and it models no authored reservations, no per-kind namespaces, and no save schema (which it leaves to the product). Adopting it would add component, containment, and revision machinery to an id space the Engine never issued. Revisit only if the product begins using `EntityWorld` for its own entity storage.
