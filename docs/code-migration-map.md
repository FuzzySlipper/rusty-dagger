...
a local manifest persistence authority. At Engine
`384dd2b`, Appearance exposes only a single sprite UV rectangle and Animation
is for animated GLB/mesh playback; no safe C# 2D atlas/frame surface exists.
The required atlas definition, frame selection/replacement/readout, per-frame
size, and admitted playback controls are tracked by upstream Engine **#7564**;
the runtime-identical preview/playback portion cannot proceed until it lands.
...
apply renderer diffs or substitute animated-mesh APIs. #7543 stops that portion
at the missing safe boundary rather than adding DOM/C# playback. This is
