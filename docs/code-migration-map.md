...
| --- | --- |
| `src/WorldRpg.Kit/**` | Safe Kit project with typed composition, Mechanics-backed actor lifetime, configured controls/input frames, Engine-default spatial scene stepping plus only explicit ruleset controller overrides, progression, bounded facts, structured UI values, and revision-guarded inventory/equipment coordination. Look and controller commands are staged then validated by Engine before product state commits; spatial receipts retain transform, motion, and ground continuation. Optional support/obstacle values are call-local facts re-submitted for each proposal, not a Kit registry or Engine-retained product state; Daggerfall currently supplies none. It references Engine only and contains no bundle resolver or Daggerfall vocabulary. |
...
| `Facts/*`, `Modules/Combat/*`, `Presentation/*` | Daggerfall combat facts/policy and Daggerfall presentation meaning. #7324 adopts direct exact-id player/rat/skeletal melee through Engine Mechanics: equipment truth is read from Engine, stamina spend and health damage are guarded Engine mutations, and damage/death/rewards are fact-ordered. #7540 adds only ordinary player melee admission: current product-owned actor snapshots become one bounded Engine Perception query over the admitted spatial session, and Daggerfall filters defeated/stale actors then ranks visible results by Engine distance and stable entity-id tie break before routing through that same explicit resolver. The copied Engine request/receipt is retained as attempt evidence; Engine owns inclusive range, aim-cone, and occlusion classification. Daggerfall-owned reach/cone tuning bounds the query while action reach remains authored policy. It still rejects encounter activation, autonomous enemy/AI loops, local ray/distance/cone helpers, and target registries. |
...
| `gameplay/**` and remaining root Rust/package surfaces | Donor or retired-runtime evidence, not active implementation. The Studio/HTTP/Angular topology is retired. |
...
| `src/Dagger.Game/Modules/PlayerControl/PlayerControlState.cs` | Player control state/tuning. | **Adapted to Kit** as generic transform/motion/ground continuity and sparse controller override contract; Daggerfall owns its supplied values. |
| `src/Dagger.Game/Modules/PlayerControl/PlayerInputSystem.cs` | Input interpretation and Engine Look use. | **Adapted to Kit** as finite typed input interpretation and Look-before-motion; Daggerfall owns bindings and semantic actions. |
| `src/Dagger.Game/Modules/PlayerControl/SpatialMovementSystem.cs` | Movement policy and Engine Spatial use. | **Adapted to Kit** as normalized-artifact admission, Engine controller proposal, receipt continuation, and reverse cleanup; Daggerfall owns explicit controller tuning. |
...
| --- | --- | --- | --- |
| `Cargo.toml`, `Cargo.lock`, `crates/{arena2,dagger-import,dagger-rpg,dagger-runtime}/Cargo.toml` (6 remaining Cargo graph files) | Workspace membership, legacy dependency topology, command names, and only the source-family boundaries below. | **Reject** the Rust workspace/package topology. C# projects and the Engine-managed SDK are the active product; no Cargo analogue is created. | #7543 removed the retired Studio-adapter member. #7544 deletes these remaining graph files only after every row below is adopted, adapted, deferred with retained content/provenance, or rejected. `Cargo.lock` has no semantic destination. |
...
| `crates/dagger-runtime/src/{animation,combat_assets,directional,lib,navgrid,patrol,player,project,runtime}.rs`, `crates/dagger-runtime/src/bin/{dagger-derive-route,dagger-gameplay-check,dagger-navgrid,dagger-sprite-frames,dagger-walkthrough}.rs` (14) | Project reading, directional/sprite frame meaning, authored combat asset validation, navigation evidence, player/control semantics, patrol experiments, and command-line diagnostic vectors. Animation is a useful *semantic* frame-layout/timing donor, not a replacement renderer or clock. | **Adapt** reusable controls/actor/spatial coordination only into **WorldRpg.Kit** where ownership is uncertain; put Daggerfall timing/asset meaning in the ruleset/content/tuning. #7540 adapts only the target-admission intent through Engine Perception; its copied range/facing/occlusion receipt feeds Daggerfall eligibility/ranking and then existing combat policy. **Reject** the Rust aggregate runtime, local loop/clock, renderer diffs, patrol/encounter runtime, command topology, and local targeting/raycast helpers. | #7539 establishes Kit/Engine routing; #7540 has bounded ordinary player target acquisition over verified Engine queries; #7541 ports behavior/presentation semantics. Each binary is deleted after its useful vector is a C# importer/ruleset fixture or explicitly obsolete. |
| Former `crates/dagger-studio-adapter/**` (13 source files plus its Cargo manifest) | Content-addressed resource/provenance checks, developer readouts, sprite/atlas inspection, diagnostic overlays, semantic product presentation, and the old Studio/product-server protocol. | **Adapted then retired by #7543.** `Daggerfall.Import` owns normalized provenance/authoring inspection, `WorldRpg.SpriteWorkbench` owns the supported sprite management workflow, and Engine Content/Appearance/UI own resource, display, playback, and projection mechanisms. The Rust protocol/manual server, retained-frame renderer, local animator, and browser/product authority are rejected. | The source family and its workspace member are deleted. Engine #7564/#7649 supplied the required safe atlas/playback controls; focused C# workbench, Import, and Engine-backed presentation tests are the replacement evidence. |
...
| Residual encounter surfaces: `data/encounters/{privateers-hold,encounter-gallery}.json`; encounter generation/launch/browser assertions in the remaining legacy scripts | Migration debris from the rejected grouping/demo system. Any independently useful sprite/frame-layout evidence has been rehomed under the normalized importer and sprite workbench without encounter identity. | **Reject; never migrate.** #7537 removed active normalized schema/data/reader. #7543 deleted the stale Angular/Studio gallery projects and tooling; #7544 deletes the remaining generators, data, launch paths, and obsolete assertions. | Deletion proof searches active C#, normalized payloads, generated content, UI, scripts, and tests—not only Rust/`gameplay/`. No empty compatibility field or gallery survives merely to satisfy legacy proof. |
| Former `apps/dagger-product/**` (15 tracked Angular app files) | The stale Angular product shell, semantic actions/readouts, developer tools, and the **sprite viewer/editor**: asset index, atlas/frame inspection, manifests, and save workflow. | **Adapted then retired by #7543.** The Engine-hosted `WorldRpg.SpriteWorkbench` and `src/sprite-ui` preserve sprite inspection, animation preview/control, validated overlay save, and projection/action workflows. Angular runtime/proxy/API bridge, UI-owned render frame/sprite playback, and Studio transport are rejected. | The app and its stale broker configurations are deleted. The workbench uses the shared Engine Content/Appearance/UI path and its focused C# tests replace the old application/bootstrap proof. |
...
The former Angular sprite panel was evidence for a useful tool, not its
authority. #7543 has now preserved its list/index, atlas/frame inspection,
animation preview/control, validated overlay-edit, and save workflows in
`WorldRpg.SpriteWorkbench` with the thin `src/sprite-ui` projection/action
surface. `Daggerfall.Import` owns normalized source/provenance interpretation;
Engine Content, Appearance, and UI own resource admission, display, playback,
and projection delivery. Engine #7564/#7649 supplied the required safe atlas,
playback, and synchronized-selection controls. The removed UI never becomes a
timer, atlas packer, resource server, renderer-diff path, or persistence
authority.
...
| Rust targeting/raycast/sensing evidence | Daggerfall player melee target choice/admission now uses one Engine Perception request over current product actor observations, exact copied classifications, Daggerfall eligibility/ranking, and the shared explicit combat resolver. No encounter scaffold, local target geometry, or AI behavior is revived. | #7540 |
...
| `dagger-studio-adapter`, HTTP server, stale Angular app, and gameplay package authoring | Studio/HTTP/Angular topology is retired by #7543 after diagnostics, provenance, and sprite authoring moved to `Daggerfall.Import`, `WorldRpg.SpriteWorkbench`, thin DOM projections, and Engine Content/Appearance/UI. The gameplay package/materializer topology is rejected and remains #7544 deletion work. | #7543; #7544 for remaining package/gameplay deletion |
