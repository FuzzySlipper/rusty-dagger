...
mod tests {
    ...
    #[test]
    fn committed_catalog_resolves_clone_assets_by_semantic_identity() {
        let catalog = CombatAssetCatalog::from_json(CATALOG).expect("committed combat catalog");
        let weapon = catalog
        ...
        assert_eq!(
            catalog.effect("effect.blood.0").unwrap().sprite_asset_id(),
            "sprite/effect-blood-0-atlas"
        );
        assert!(catalog.audio("audio.melee.dagger.swing").is_some());
        assert!(catalog.audio("audio.melee.hit.5").is_some());
    }
}
