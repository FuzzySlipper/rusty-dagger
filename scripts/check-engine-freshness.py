#!/usr/bin/env python3
"""Fail when Rusty Dagger's rolling Engine facade lock is not current."""
...
DEFAULT_REPOSITORY = "https://github.com/FuzzySlipper/rusty-engine.git"
...
def manifest_revision(manifest: Path, repository: str, branch: str) -> str:
    ...
    if specification.get("package", alias) != "rusty-engine":
        raise ValueError("the canonical Engine dependency must be rusty-engine")
    if specification.get("branch") != branch or "rev" in specification or "tag" in specification:
        raise ValueError(f"rusty-engine must follow rolling branch {branch!r}")
    metadata = workspace.get("metadata", {}).get("rusty-engine", {})
    if metadata.get("repository") != repository:
        raise ValueError("workspace Engine metadata must name the canonical repository")
    revision = metadata.get("revision")
    if not isinstance(revision, str) or re.fullmatch(r"[0-9a-f]{40}", revision) is None:
        raise ValueError("workspace Engine metadata revision must be a 40-character SHA")
    return revision
...
def resolved_revision(lockfile: Path, repository: str, branch: str) -> str:
    ...
    packages = [
        package for package in lock.get("package", []) if package.get("name") == "rusty-engine"
    ]
    if len(packages) != 1:
    ...
    if not isinstance(source, str) or not source.startswith(expected_prefix):
        raise ValueError(f"rusty-engine lock source must use {expected_prefix}<sha>")
    revision = source.removeprefix(expected_prefix)
    if re.fullmatch(r"[0-9a-f]{40}", revision) is None:
        raise ValueError("rusty-engine lock source must end in a 40-character SHA")
    return revision
...
def main() -> int:
    args = parse_args()
    try:
        declared = manifest_revision(args.manifest.resolve(), args.repository, args.branch)
        resolved = resolved_revision(args.lockfile.resolve(), args.repository, args.branch)
        if declared != resolved:
            raise RuntimeError(
                f"Engine metadata/lock split: metadata {declared}, lock {resolved}"
            )
