...
const validHud = {
  ...
  activeEncounter: { name: 'Rat Cellar', objective: 'Defeat the rat.' },
  lastOutcome: 'Exploring',
};

assert(isHud(validHud), 'expected a complete HUD projection to be accepted');
assert(!isHud({ ...validHud, activeEncounter: { name: 'Rat Cellar' } }), 'expected an incomplete encounter to be rejected');
assert(!isHud({ ...validHud, activeEncounter: 'Rat Cellar' }), 'expected a non-object encounter to be rejected');
