...
exec cargo run --manifest-path "$engine/rust/crates/csharp-product-runtime/Cargo.toml" --locked -- \
  --library "$root/native/RustyDagger.NativeProduct.so" --bundle-dir "$root/browser-bundle" --content-dir "$repo/content" --port "${RUSTY_DAGGER_PORT:-4394}" --mode realtime --direct-intent attack=digital
